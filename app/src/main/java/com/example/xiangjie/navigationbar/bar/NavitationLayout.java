package com.example.xiangjie.navigationbar.bar;
...
public class NavitationLayout extends RelativeLayout{
    ...
    public void setNavLine(Activity context, int height, int color, int currentPosition)
    {
        if(textViews != null)
        {
            navWidth = dip2px(context,300)/ textViews.length;
            Toast.makeText(context, "navWidth:---"+navWidth+"--"+getWidth()+"--"+textViews.length
                    , Toast.LENGTH_SHORT).show();
            Log.e("test","navWidth:---"+navWidth+"--"+getWidth()+"--"+textViews.length
            );
        }
